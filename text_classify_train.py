...
def mymodel_train(args, logger, train_dataloader, validation_dataloader):
    ...
    for _ in trange(epochs, desc='Epochs'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            b_input_ids, b_input_mask, b_labels = batch  # 获取一个batch的训练数据
            optimizer1.zero_grad()  # 优化器梯度清零
            optimizer2.zero_grad()
            text_embedding = embedding(input_ids=b_input_ids.squeeze(1).long(),  # 通过语言模型得到文本矩阵
                                       attention_mask=b_input_mask)
            loss, tmp_eval_accuracy = model(text_embedding, b_labels, b_input_mask.squeeze(1))  # 关键！核心！
            loss.backward()  # 反向传播
        ...
        adjust_learning_rate(optimizer1, 0.9)  # 调整优化器学习率
...
def get_dataloader(filenames):
    tokenizer = ElectraTokenizer.from_pretrained(args.vocab_dir)  # 定义tokenizer
